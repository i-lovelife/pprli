...
logger = logging.getLogger('ae_ganr')
logger.setLevel(logging.INFO)
"""
Test whether wgan influence result
"""
...
@main.command()
@click.option('--name', type=str, default='ae_ganr0')
...
def train(name, 
    ...
    loader = load_ferg()
    ae_ganr = AeGanR(loader, debug=debug)
    if recover_dir is not None:
        ae_ganr.load_weights(recover_dir)
    ae_ganr.train(experiment_dir, total_iter=epoch, batch_size=batch_size, sample_iter=sample_iter, model_save_iter=model_save_iter)
    if test:
        acc_y_on_x = ae_ganr.evaluate_y_on_x()
        acc_p_on_x = ae_ganr.evaluate_p_on_x()
...
def test(name, iter_no=100, debug=False, num_epochs=20):
    ...
    loader = load_ferg()
    ae_ganr = AeGanR(loader, debug=debug)
    ae_ganr.load_weights(experiment_dir=experiment_dir, iter_no=iter_no)
    acc_y_on_x = ae_ganr.evaluate_y_on_x(num_epochs=num_epochs)
    acc_p_on_x = ae_ganr.evaluate_p_on_x(num_epochs=num_epochs)
    logger.info(f'acc_y_on_x = {acc_y_on_x}, acc_p_on_x={acc_p_on_x}')
